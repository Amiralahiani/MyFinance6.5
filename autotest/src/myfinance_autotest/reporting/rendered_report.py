...
def _markdown(report: ApiPrototypeReport) -> str:
    lines = [
        f"# Rapport autonome — {report.test_id}",
        "",
        f"- Verdict : **{report.verdict.value.upper()}**",
        f"- Endpoint : `{report.endpoint}`",
        f"- Durée : {report.duration_ms} ms",
        f"- Catégories d’anomalie : {', '.join(item.value for item in report.failure_categories) or 'aucune'}",
        "",
        "## Contrôles déterministes",
        "",
        "| Contrôle | Résultat | Attendu | Observé |",
        "| --- | --- | --- | --- |",
    ]
    ...
    if report.grounding:
        lines.extend(["", "## Preuve financière"])
            ...
            for evidence in report.grounding.evidence:
                lines.extend(
                    [
                        ...
                        f"- Extrait : {evidence.excerpt}",
                    ]
                )
        else:
            lines.append("Aucune preuve auto-validée disponible.")
    if report.evaluation:
        lines.extend(
            [
                "",
                "## Évaluation qualitative Groq",
                "",
                f"Verdict qualitatif : **{report.evaluation.verdict.value.upper()}** · pertinence {report.evaluation.relevance}/5 · clarté {report.evaluation.clarity}/5.",
                report.evaluation.rationale,
            ]
        )
    if report.critic_decision:
        lines.extend(["", "## Décision Critic", "", report.critic_decision.reason])
    if report.regression:
        lines.extend(["", "## Régression enregistrée", "", f"`{report.regression.regression_id}`"])
    return "\n".join(lines) + "\n"
...
def write_rendered_api_report(report: ApiPrototypeReport, root: Path) -> tuple[Path, Path]:
    ...
    html_path.write_text(
        ...
        "<title>Rapport autonome</title><style>body{font-family:system-ui;margin:2rem;max-width:1000px}pre{white-space:pre-wrap}</style>"
